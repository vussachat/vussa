use axum::{Json, http::{HeaderMap, HeaderValue, StatusCode}, response::{IntoResponse, Response}};
...
impl From<redis::RedisError> for AppError {
    fn from(error: redis::RedisError) -> Self { Self::bad_request(error.to_string()) }
}

impl From<sqlx::Error> for AppError {
    fn from(error: sqlx::Error) -> Self {
        tracing::error!(%error, "database request failed");
        Self { status: StatusCode::INTERNAL_SERVER_ERROR, message: "database request failed".to_string() }
    }
}

impl From<RepositoryError> for AppError {
    fn from(error: RepositoryError) -> Self { Self::bad_request(error.to_string()) }
}
...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        ...
        (self.status, headers, Json(serde_json::json!({"error": self.message}))).into_response()
    }
}
...
mod tests {
    ...
    fn rate_limit_errors_include_retryable_status() {
        assert_eq!(AppError::too_many_requests("busy").status, StatusCode::TOO_MANY_REQUESTS);
    }
